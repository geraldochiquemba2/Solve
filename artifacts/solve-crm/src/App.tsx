...
function CustomersPage({ customers, loading }: { customers: Customer[]; loading?: boolean }) {
  ...
  const filtered = customers.filter(c => `${c.name} ${c.company} ${c.id}`.toLowerCase().includes(q.toLowerCase()) && (status === 'Todos' || c.state === status));
