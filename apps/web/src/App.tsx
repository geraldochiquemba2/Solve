...
function Fit90LeadsPage({ leads, onChanged }: { leads: Lead[]; onChanged?: () => void }) {
  ...
  const move = (lead: Lead, apiStatus: string) => updateMut.mutate({ id: lead.id, data: { status: apiStatus as any } }, { onSuccess: () => onChanged?.() });
  const filtered = leads.filter(l => (l.name + l.phone + l.email + l.code).toLowerCase().includes(q.toLowerCase()));
  const wa = (phone: string) => { const d = phone.replace(/[^\d]/g, ''); return d.length >= 9 ? `https://wa.me/${d}` : ''; };
  const fmtDate = (d: string) => { if (!d) return '—'; try { return new Date(d).toLocaleDateString('pt-AO', { day: '2-digit', month: 'short', year: 'numeric' }); } catch { return d; } };
  const detail = (notes: string) => ({ plan: notes.match(/Plano:\s*([^,]+)/i)?.[1]?.trim() ?? '', freq: notes.match(/Frequência:\s*([^,]+)/i)?.[1]?.trim() ?? '' });
  const ttl = (createdAt: string) => {
    if (!createdAt) return <span>—</span>;
    const left = 72 * 3600e3 - (Date.now() - new Date(createdAt).getTime());
    if (left <= 0) return <span style={{ color: 'hsl(3 67% 45%)' }}>expirado</span>;
    const h = Math.floor(left / 3600e3); const m = Math.floor((left % 3600e3) / 60000);
    const urgent = left < 24 * 3600e3;
    return <span style={{ color: urgent ? 'hsl(28 82% 42%)' : 'hsl(var(--muted-foreground))' }}>expira em {h > 0 ? `${h}h ${m}m` : `${m}m`}</span>;
  };
  return <><PageHeader eyebrow="Comercial · Captação" title="Leads Fit90" subtitle={`${leads.length} cadastros registados na landing Fit90`} /><div className="card" style={{ padding: '.7rem', marginBottom: '.8rem', display: 'flex', gap: '.55rem', alignItems: 'center', flexWrap: 'wrap' }}><div style={{ position: 'relative', flex: '1 1 230px' }}><Search size={14} style={{ position: 'absolute', left: 10, top: 10, color: 'hsl(var(--muted-foreground))' }} /><input className="input" style={{ paddingLeft: 31 }} placeholder="Pesquisar por nome, WhatsApp, email ou código" value={q} onChange={e => setQ(e.target.value)} data-testid="input-search-fit90" /></div></div><Section title="Cadastros Fit90" note={`${filtered.length} registos · duração máxima de 72h`}>{filtered.length === 0 ? <EmptyState title={q ? 'Sem resultados' : 'Nenhum cadastro Fit90' } text={q ? 'Tenta outra pesquisa.' : 'Os leads submetidos na landing Fit90 (fit90-leads) aparecerão aqui automaticamente.'} /> : <div className="table-wrap"><table className="data-table"><thead><tr><th>Código</th><th>Nome</th><th>WhatsApp</th><th>Email</th><th>Plano / Frequência</th><th>Valor estimado</th><th>Etapa</th><th>Data de registo</th><th /></tr></thead><tbody>{filtered.map(l => { const d = detail(l.notes); return <tr key={l.id} data-testid={`row-fit90-${l.id}`}><td className="mono" style={{ fontSize: '.68rem', color: 'hsl(var(--muted-foreground))' }}>{l.code || '—'}</td><td style={{ fontWeight: 600 }}>{l.name || '—'}</td><td>{l.phone ? <a href={wa(l.phone)} target="_blank" rel="noreferrer" className="btn-quiet" data-testid={`link-whatsapp-${l.id}`}><MessageCircle size={13} style={{ verticalAlign: 'middle' }} /> {l.phone}</a> : '—'}</td><td style={{ fontSize: '.75rem' }}>{l.email || '—'}</td><td style={{ fontSize: '.72rem', color: 'hsl(var(--muted-foreground))' }}>{(d.plan && `Plano: ${d.plan}`)}{d.plan && d.freq ? ' · ' : ''}{d.freq}</td><td className="mono" style={{ fontSize: '.75rem' }}>{l.value ? money(l.value) : '—'}</td><td><Status tone={l.status === 'Convertido' ? 'good' : l.status === 'Perdido' ? 'danger' : l.status === 'Novo Lead' ? 'pending' : 'neutral'}>{l.status || 'Novo Lead'}</Status></td><td style={{ fontSize: '.72rem', color: 'hsl(var(--muted-foreground))' }}>{fmtDate(l.createdAt)}<div style={{ fontSize: '.62rem', marginTop: '.15rem' }}>{ttl(l.createdAt)}</div></td><td>{l.status === 'Novo Lead' ? <button className="btn-quiet" data-testid={`button-contactar-${l.id}`} onClick={() => move(l, 'contacto')}><PhoneCall size={13} style={{ verticalAlign: 'middle' }} /> Marcar contacto</button> : <span style={{ fontSize: '.68rem', color: 'hsl(var(--muted-foreground))' }}>Em acompanhamento</span>}</td></tr>; })}</tbody></table></div>}</Section></>;
}
function CustomersPage({ customers, loading, onChanged }: { customers: Customer[]; loading?: boolean; onChanged?: () => void }) {
  ...
  const ovgAge = ovg.lastSync ? Math.max(0, Math.round((Date.now() - new Date(ovg.lastSync).getTime()) / 60000)) : null;
