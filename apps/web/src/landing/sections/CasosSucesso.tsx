        ...
        >
          <AnimatePresence mode="wait">
            ...
            >
              <div className="grid md:grid-cols-[1fr_2fr] gap-8 items-start">
                ...
                <div>
                  {/* Photo placeholder */}
                  <div className="w-20 h-20 bg-gradient-to-br from-[#042251]/10 to-[#042251]/20 border border-[#042251]/20 flex flex-col items-center justify-center mb-4 glow-red">
                    <span className="font-heading font-black text-2xl text-[#042251]">
                      {activeCase.name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                    </span>
                    <span className="text-[7px] font-bold text-white bg-[#042251] px-1 mt-1 text-center leading-tight">
                      [ FOTO ]
                    </span>
                  </div>
                  ...
                  <div className="font-body text-[#042251] text-sm font-semibold mb-4">{activeCase.company}</div>

                  {/* Tags */}
                  <div className="flex flex-wrap gap-2 mb-4">
                    {activeCase.tags.map((tag) => (
                      <span key={tag} className="px-3 py-1 bg-[#042251]/10 border border-[#042251]/20 text-[#042251] text-xs font-body font-semibold">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  {/* Result highlight */}
                  <div className="flex items-center gap-3 mb-6 p-4 bg-[#042251]/10 border border-[#042251]/20">
                    <div className="w-2 h-2 bg-[#042251] flex-shrink-0" />
                    <span className="font-heading font-bold text-[#042251] text-lg">{activeCase.result}</span>
                  </div>

                  <Quote size={32} className="text-[#042251]/20 mb-4" />
          ...
          <div className="flex items-center justify-between mt-8">
              ...
              {cases.map((_, i) => (
                <button
                  ...
                  className={`transition-all duration-300 ${
                    i === current ? 'w-8 h-2 bg-[#042251]' : 'w-2 h-2 bg-zinc-300 hover:bg-zinc-400'
                  }`}
            ...
            <div className="flex gap-3">
              <button
                onClick={prev}
                className="w-11 h-11 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-zinc-900 dark:text-white hover:border-[#042251]/40 transition-all"
                aria-label="Anterior"
              >
                <ChevronLeft size={18} />
              </button>
              <button
                onClick={next}
                className="w-11 h-11 bg-[#042251] flex items-center justify-center text-white hover:bg-[#0A3A75] transition-all"
                aria-label="Próximo"
              >
